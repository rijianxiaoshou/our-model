...
def parse_args():
    ...
    # loss
    parser.add_argument('--loss', default='CE+focal', choices=LOSS_NAMES)
# 学习非传统交叉熵
    # dataset
    ...
    parser.add_argument('--lr', '--learning_rate', default=2e-04, type=float,
    ...
    parser.add_argument('--min_lr', default=5e-05, type=float,
...
def main():
    ...
    if config['pretrained']:
        model = SwinConv(hidden_dim=config['dim'], layers=(2, 2, 6, 2), heads=(3, 6, 12, 24),
                         num_classes=128)
        model = LoadPretrain_SimCLR(pretrained_path=config['pretrained'], model=model)
    else:
        model = SwinConv(hidden_dim=config['dim'], layers=(2, 2, 6, 2), heads=(3, 6, 12, 24),
                         num_classes=config['num_classes'])
